                    ...
                    for i, job in enumerate(matched_jobs[:8], 1):
                        ...
                        with st.expander(f"{emoji} #{i} **{job['job']}** - Match: **{score}%**"):
                            ...
                            if role_skills:
                                ...
                                st.info(", ".join(role_skills[:10]))
                else:
                    st.warning("⚠️ No job matches found. Try adding more relevant keywords.")
            
            # ==================== TAB 2: ATS SCORE ====================
            with tab2:
                st.markdown("### 📊 ATS (Applicant Tracking System) Compatibility")
                st.caption("Estimated ATS compatibility scores for top roles")
                
                if matched_jobs:
                    st.info("💡 **Tip:** Scores above 70% are considered excellent for ATS systems")
                    ...
                    st.markdown("""
                    1. **Use Keywords**: Include exact terms from job descriptions
                    2. **Standard Format**: Avoid tables, graphics, headers/footers
                    3. **Clear Sections**: Use standard headings (Experience, Education, Skills)
                    4. **Spell Out Acronyms**: Use both forms (e.g., "Artificial Intelligence (AI)")
                    5. **Quantify Results**: Include numbers and metrics
                    """)
                else:
                    st.warning("Upload a resume to view ATS scores")
...
6. **Apply Strategically**: Tailor applications to job descriptions
            ...
            # ==================== TAB 5: ENHANCED PDF ====================
            with tab5:
                st.markdown("### 📥 Generate Enhanced Resume PDF")
                ...
                if not PDF_AVAILABLE:
                    ...
                    st.success(f"✅ Ready to generate enhanced resume for: **{analysis['role']}**")
                    ...
                    if st.button("🎨 Generate Enhanced PDF", type="primary", use_container_width=True):
                        with st.spinner("Creating your enhanced resume PDF..."):
                            try:
                    ...
                    st.info("👆 First, analyze your resume in the **Skill Enhancement** tab")
    ...
    **🎯 Job Matching**
    - Find best job roles for your profile
    - Get direct links to job boards
